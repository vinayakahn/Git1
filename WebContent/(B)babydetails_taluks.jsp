...
<html>
	<head>
   	...
   	function display(index)
   	{
   		...
   		var xmlDoc, txt, x, i;
   		var response="";
   		var urls="facility_exist.jsp?tq="+index;
   		...
   		/* 	  xmlDoc = this.responseXML;
   			  ...
   			  for (i = 0; i < x.length; i++) {
   			    txt = txt + x[i].childNodes[1].nodeValue + "<br>";
   			  }
    			 document.getElementById("demo").innerHTML = txt;
   			  } */
   				
            //var val=roott.getElementsByTagName("value")[z].childNodes[0].nodeValue;
   			var z=0;
 	 			alert("ajax4")
             var roott=xmlhttp.responseXML.documentElement;
              alert("ajax5")
        	  alert("ajax2"+roott);
              var text=roott.getElementsByTagName("TEXT")[z].childNodes[0].nodeValue;
              alert("ajax3"+text);
              while(text!=null)
              {
                      addoptions(text)
    ...
    function addoptions(text)
    {
        ...
        ct1.text=text;
        ct1.value=text;
        document.babydetails.facility1.options.add(ct1);
 
    }
     function removeall()
        {

            var ct=document.babydetails.facility1.length;
            for(i=ct; i>=0; i--)    {    
                document.babydetails.facility1.options[i]=null;
                 }
        }
    function fun(index)
    {
	...
	<body>
		...
		<div class="container" align="center">		 
			<form action="babydetails_taluks.jsp" method="POST" name="babydetails" class="form-inline">
			<h1 style = "color:brown"> Baby Details </h1>
				...
				<select class="form-control" id="facility" name="facility" style="padding:3px;" onchange="display(this.options.selectedIndex)">
					...
					<option value=1>koppal_villages</option>
					<option value=2>gangawati_villages</option>
					<option value=3>kushtagi_villages</option>
					<option value=4>yelbarga_villages</option>
					<option value=5>Others</option>
				</select>
		 	</div>
		 	<div class="form-group">
				<div id="filters2"></div>
					<label style="font-size:20px; color:blue"> Facility: </label> 
				<select class="form-control" id="facility1" name="facility1" style="padding:3px;">
					<option value="">Choose</option> 	
				</select>
		 	</div>
		 	<div class="form-group">							
		...
		<%
			...
			try
			{
			    ...
			    //convert string to int;
			  	...
			  	int taluks = Integer.parseInt(taluk);
			   ...
			   //date covert to yyyy-mm-dd format
				...
				ArrayList<DBObject> jsonArray = mongodao.babyFacilityTaluk(fno,date_from,date_to);
